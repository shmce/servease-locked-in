...
export const backendSupportMatrix: BackendSupportItem[] = [
  ...
  },
  {
    ...
    status: "partial",
    currentSupport: "Profile and password updates call shared account backend endpoints. Sessions endpoint now returns Supabase auth.users.last_sign_in_at-derived session record. 2FA stubs still return 501.",
    existingEndpoints: [
      "GET /v1/me",
      "PATCH /v1/me",
    ...
    backendNeeded: [],
    notes: "Sessions endpoint is now wired to auth.users via servease_list_user_sessions RPC. 2FA stubs still return 501 - TOTP/SMS requires a dedicated auth contract.",
  },
  {
    area: "Reports",
